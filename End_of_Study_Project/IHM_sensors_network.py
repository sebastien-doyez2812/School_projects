...
fig1, axis1 = plt.subplots(figsize=(6, 1), dpi = 80)
...
fig2, axis2 = plt.subplots(figsize=(6, 1) , dpi = 80)
...
fig3, axis3 = plt.subplots(figsize=(6, 1), dpi = 80)
...
def read_data_worker():
    global time_list, dataX_1, dataY_1, dataZ_1, id_sensor_selected
    global window
    time_list.clear()
    dataX_1.clear()
    dataY_1.clear()
    ...
    # The user can choose the sensor:
    text_user = Label(window, text= "Sensor selected:")
    text_user.pack() 

    id_spinbox = Spinbox(window, from_=1, to= nb_sensor)
    id_spinbox.pack()

    while True:
        # We need to know which sensor is selected:
        id_sensor_selected = int(id_spinbox.get())
        ...
        with open("data.txt", 'r') as file:
            ...
            # While we are not at the end of the data.txt... 
            while index < len(data_txt):
                ...
                if len(donnee) == 5 * nb_sensor and donnee[5*(int(id_sensor_selected)-1)] != "ERROR":    # if len(donnee) => there are 2 errors 
                    
                    # Here, the line constains the data
                    time_list.append(float(int(donnee[0+ 5*(int(id_sensor_selected) -1)]) * 1e-6)) # if the sensors selected is the n°2 => the data start at 5*(id_sensor_selected -1)
                    dataX_1.append(float(donnee[1+ 5*(int(id_sensor_selected) -1)])) 
                    dataY_1.append(float(donnee[2 + 5*(int(id_sensor_selected) -1)])) 
                    dataZ_1.append(float(donnee[3 + 5*(int(id_sensor_selected) -1)])) 
...
def Show_data():
    ...
    # First, We need to select the id of the sensor:
    text00 = Label(Select_window, text= "Select the sensor")
    text00.pack()

    id_selected = Spinbox(Select_window, from_=1, to= nb_sensor)
    id_selected.pack()
    ...
    def read_csv(nom_fichier, Y_chosen):
        ...
        with open(nom_fichier, 'r') as fichier_txt:
            ...
            for ligne in lines:
                ...
                if len(data) == 5 * nb_sensor and data[5*(int(id)-1)] != "ERROR":
                    
                    # A way to know if we take the data is to use %: if i%f == 0, we need to plot the data.
                    if i%f == 0:
                        t_list.append(int(data[5*(int(id)-1)]))  
    ...
    def Sensor_selector():
        ...
        id = id_selected.get()
        if data == 'X data':
            Y_chosen = 5*(int(id)-1) + 1
            title_focus = "Graph on the X axis"
        elif data == 'Y data':
            Y_chosen = 5*(int(id)-1) + 2
            title_focus = "Graph on the Y axis"
        else:
            Y_chosen = 5*(int(id)-1) + 3
        ...
        ax_select.set_xlabel("Time [us]")
        ax_select.set_ylabel("Acceleration [mg]")
...
def test_integrite():
    previous_id =   np.zeros(nb_sensor)
    previous_time = np.zeros(nb_sensor)
    time_missed =   np.zeros(nb_sensor)
    ...
    with open ("data.txt", 'r') as file:
        ...
        #The first and last data are skipped because there are sometimes there are errors due to the connection of CoolTerm
        for j in range(len(data_txt)- nb_val, len(data_txt)-5, 1):
            ligne = data_txt[j]
            data = ligne.split()

            # We need to test for earch sensors:
            for i in range(nb_sensor):

                # If this is the firt data, we need to store the previous id and the previous time:
                if j == len(data_txt) - nb_val and data[5*i] != "ERROR":
                    previous_id[i] = int(data[4 + 5 * i])
                    previous_time[i] = int(data[0 + 5 * i])
                    continue
                # Else, this is not the first data, so we can find if there are errors:
                    ...
                    if len(data) == 5 * nb_sensor : # Test if there are no misss on the list data 
                        if data[5*i] != "ERROR" : 
                            # Because the errors from teensy can be spotted with the first data in the line : "ERROR"
                            id = int(data[4 + 5 * i])
                        ...
                        if data[5 * i] == "ERROR": 
                            ...
                            with open("error_report.txt", 'a') as file_error_report:
                                ...
                                # code 2: data repetition
                                if code == 0:
                                    file_error_report.write( f"\n\t\t\t\t\t\t\tERROR TEENSY sensor id {i +1 }:\t code 0: All data are 0.\n") 
                                else :
                                    if code == 1:
                                        file_error_report.write( f"\n\t\t\t\t\t\t\tERROR TEENSY sensor id {i +1 }:\t code 1: There is too much delay between 2 data.\t") 
                                        #time_missed += float(data[3]) - 100 # The delay should be 100 us.
                                    else :
                                        if code == 2:
                                            file_error_report.write( f"\n\t\t\t\t\t\t\tERROR TEENSY sensor id {i +1 }:\t code 2: There is a data repetition.\n")                  
                            file_error_report.close() 
        ...
        def save_error_report():
            ...
            # Check if the file exists
            if not os.path.exists(file_path):
                messagebox.showerror("Error", "The file error_report.txt do not exist.")
                return
                # Build the path with os.path.join
...
def frequency():
    ...
    # The frequency is here calculate with the first sensor's data
    previous = 0
    sum = 0
...
def Help_IHM():
    ...
    help_label = Label(help_window, text="This interface has been created with the aim of simplifying the use of the layout as well as the management of output data.\n\n The first page show 3 graphs of the X, Y & Z axis of the sensor selected, You can selected another with the spinbox.\nYou can show all the data with edit > Show Data: selected the axis; id; frequency of the pllot.\n")
    help_label.pack()

    help_label2 = Label(help_window, text=" You can also check the integrity ( control > test integrity ): you will have a error report, or check the frequeency (control > frequency).\n\n If you want to save the data: File > Export the data to a CSV file, and choose a folder.")
    help_label2.pack()
...
def About_us():
    ...
    """
    showinfo("About us", "This interface was created by Sébastien Doyez, a MEA5, in 2023, with the help of the Géoscience Montpellier Team ... \nIf you need further information, you can contact him at the following address: \nsebastien.doyez@etu.umontpellier.fr")
